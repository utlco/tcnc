...
class SVGPreviewPlotter(gcode.PreviewPlotter):
    ...
    def __init__(self, svg_context, tool_offset=0.0, tool_width=0.0,
        ...
        nonzero_toolwidth = not geom.is_zero(self.tool_width)
        ...
        # DEBUG
        self.debug_layer = self.svg.create_layer('gcodesvg debug', flipy=True)
...
#         logger.debug('len=%.4f, n=%d, aincr=%.4f, pincr=%.4f' % (seglen, num_markers, angle_incr, point_incr))
        angle = start_angle
        u = 0
        while u < (1.0 + geom.const.EPSILON):
...
#        points_B.reverse()
        ...
        # DEBUG
        self.svg.create_polypath(side_A, style=style, parent=self.debug_layer)
        self.svg.create_polypath(side_B, style=style, parent=self.debug_layer)
...
#                                 parent=self.tool_layer)
    ...
    def _draw_subpaths(self):
        ...
        while offset < length:
            ...
            path = self._simplify_path(path, self.x_subpath_maxdist)
